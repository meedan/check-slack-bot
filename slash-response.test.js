...
test('return error message if duplicated url and its url', async () => {
  ...
  pm = await callCheckApi('get', { class: 'project_media', id: pm.data.id, fields: 'oembed' });
  ...
  expect(callback).toHaveBeenCalledWith(null, expect.objectContaining({ attachments: expect.arrayContaining([expect.objectContaining({text: 'This item already exists: ' + pm.data.oembed.permalink})])}));
});
